...
# define boundaries of the plot area, default is adding on 1 km to easting
# and northing
...
# using matplot lib colors in bokeh, change colors by variable in source
# dataset
def color_by_variable(cbv, source):
    colors = ["#%02x%02x%02x" % (int(r), int(g), int(
        b)) for r, g, b, _ in 255*mpl.cm.jet(mpl.colors.Normalize()(source.data[cbv]))]
    return colors
...
def stream_map(x_range, y_range, source, x='x', y='y', plot_width=600, plot_height=300, title='Title', x_axis_label='Label', y_axis_label='Label'):
    ...
    plot = bp.figure(plot_width=plot_width, plot_height=plot_height, x_range=x_range,
                     y_range=y_range, title=title, x_axis_label=x_axis_label, y_axis_label=y_axis_label)
